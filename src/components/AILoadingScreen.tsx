...
const ImageDisplay = ({
  ...
  const reducedMotion = useMotionPreference();
  const t = useTranslations("Components.AILoadingScreen");
  ...
  const statusText =
    mode === "analyze" ? t("analysisRunning") : t("generationRunning");

  if (!displayImageUrl) {
    return (
      <div className="w-full aspect-[4/3] rounded-3xl bg-base-200 flex items-center justify-center">
        <div className="text-base-content/40">{t("noImageAvailable")}</div>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl">
      ...
      <Image
        ...
        alt={
          mode === "analyze"
            ? t("imageBeingAnalyzed")
            : t("imageBeingGenerated")
        }
        fill
